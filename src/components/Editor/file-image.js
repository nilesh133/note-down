...
import { modular_storage } from "../../firebase";
...
class SimpleImage {
  ...
  render() {
    ...
    this.wrapper.classList.add("file-image");
  ...
  _createImage(url, captionText) {
    ...
    loader.classList.add("editor-image-loader")

    const files = url.files[0];
    if (files) {
    const storageRef = ref(modular_storage, `files_images/${files?.name}`);
